...
public class Client
{
    ...
    public void UpdateNumerologySystem(string dynamic_text)
    {
        Server.UpdateNumerologySystem(dynamic_text);
    }
        ...
        try
        {
            if (NumerologySystem != null)
            {
                using (StreamWriter writer = new StreamWriter(filename, false, Encoding.Unicode))
                {
                    ...
                    if (NumerologySystem.TextMode == "Original") // force simplify Original even in Standard or Grammar Editios
                    {
    ...
    public Dictionary<string, int> GetWordsWith(string text, TextLocationInVerse text_location_in_verse, TextLocationInWord text_location_in_word, bool with_diacritics)
    {
        Dictionary<string, int> result = new Dictionary<string, int>();
        List<Verse> source = Server.GetSourceVerses(m_search_scope, m_selection, m_found_verses);
        if (Book != null)
        {
            result = Book.GetWordsWith(source, text, text_location_in_verse, text_location_in_word, with_diacritics);
        }
        return result;
    }
    public Dictionary<string, int> GetCurrentWords(string text, TextLocationInVerse text_location_in_verse, TextLocationInWord text_location_in_word, bool with_diacritics)
    {
        Dictionary<string, int> result = new Dictionary<string, int>();
        List<Verse> source = Server.GetSourceVerses(m_search_scope, m_selection, m_found_verses);
        if (Book != null)
        {
            result = Book.GetCurrentWords(source, text, text_location_in_verse, text_location_in_word, with_diacritics);
        }
        return result;
    }
    public Dictionary<string, int> GetNextWords(string text, TextLocationInVerse text_location_in_verse, TextLocationInWord text_location_in_word, bool with_diacritics)
    {
        Dictionary<string, int> result = new Dictionary<string, int>();
        List<Verse> source = Server.GetSourceVerses(m_search_scope, m_selection, m_found_verses);
        if (Book != null)
        {
            result = Book.GetNextWords(source, text, text_location_in_verse, text_location_in_word, with_diacritics);
        }
        return result;
    }
    ...
    public int FindPhrases(string text, LanguageType language_type, string translation, TextLocationInVerse text_location_in_verse, TextLocationInWord text_location_in_word, bool case_sensitive, TextWordness wordness, int multiplicity, bool with_diacritics)
    {
        ClearSearchResults();

        m_found_phrases = Server.FindPhrases(m_search_scope, m_selection, m_found_verses, text, language_type, translation, text_location_in_verse, text_location_in_word, case_sensitive, wordness, multiplicity, with_diacritics);
        if (m_found_phrases != null)
        {
            m_found_verses = new List<Verse>();
            foreach (Phrase phrase in m_found_phrases)
    ...
    /// <param name="at_word_start"></param>
    /// <returns>Number of found phrases. Result is stored in FoundPhrases.</returns>
    public int FindPhrases(string text, LanguageType language_type, string translation, ProximitySearchType proximity_search_type, bool case_sensitive, TextWordness wordness, bool with_diacritics)
    {
        ClearSearchResults();

        m_found_phrases = Server.FindPhrases(m_search_scope, m_selection, m_found_verses, text, language_type, translation, proximity_search_type, case_sensitive, wordness, with_diacritics);
        if (m_found_phrases != null)
        {
            m_found_verses = new List<Verse>();
            foreach (Phrase phrase in m_found_phrases)
    ...
    public int FindPhrases(string root, int multiplicity, bool with_diacritics)
    {
        ...
        m_found_phrases = Server.FindPhrases(m_search_scope, m_selection, m_found_verses, root, multiplicity, with_diacritics);
        if (m_found_phrases != null)
        {
            m_found_verses = new List<Verse>();
            foreach (Phrase phrase in m_found_phrases)
